...
import com.estexis.core.ui.R as CoreUiR

object OtpErrorMapper {
    fun toUiMessage(code: OtpErrorCode): UiText {
        return when (code) {
            OtpErrorCode.VALIDATION_OTP_INVALID ->
                UiText.StringResource(CoreUiR.string.error_message_invalid_otp)

            OtpErrorCode.NO_INTERNET ->
                UiText.StringResource(CoreUiR.string.error_message_no_internet)

            OtpErrorCode.USER_NOT_FOUND ->
                UiText.StringResource(CoreUiR.string.error_message_email_not_registered)
